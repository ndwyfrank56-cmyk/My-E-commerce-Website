...
def send_order_confirmation_email(email, order_id, full_name, total_amount, items):
    """Send an order confirmation email with product variations."""
    # Build items table with variations
    items_html = ""
    for item in items:
        variations = item.get('variations', '')
        variation_text = f"<br><small style='color: #999;'>{variations}</small>" if variations else ""
        items_html += f"""
        <tr>
            <td style='padding: 10px; border-bottom: 1px solid #ddd;'>{item['name']}{variation_text}</td>
            <td style='padding: 10px; border-bottom: 1px solid #ddd; text-align: center;'>{item['quantity']}</td>
            <td style='padding: 10px; border-bottom: 1px solid #ddd; text-align: right;'>RWF {item['price']:,.0f}</td>
        </tr>
        """
